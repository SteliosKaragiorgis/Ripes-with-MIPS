...
#include "../../MIPS/m5s_no_fw_hz/m5s_no_fw_hz_ifid.h"
#include "m5s_unopt_no_fw_hz_idex.h"
#include "m5s_unopt_no_fw_hz_exmem.h"
#include "../../MIPS/m5s_no_fw_hz/m5s_no_fw_hz_memwb.h"
...
class MIPS32_UNOPT_5S_NO_FW_HZ : public RipesVSRTLProcessor {
...
public:
  ...
  MIPS32_UNOPT_5S_NO_FW_HZ(const QStringList &extensions)
    ...
    1 >> idex_reg->enable;
    //idex_reg->do_jmp_out >> idex_reg->clear;
    ...
    //memwb_reg->mf_sel_out >> registerFile->mf_sel;
    ...
    */
    ...
    //br_and->out >> pc_branch->select;
    //br_and->out >> pc_src->select;
    ...
    //idex_reg->imm_out >> alu_control->instr5_0;
    ...
    //registerFile->r2_out >> data_mem->data_in;          //yes
    //control->mem_ctrl >> data_mem->op;                  // original
    exmem_reg->mem_ctrl_out >> data_mem->op;
    //control->mem_do_read_ctrl >> data_mem->mem_read;    // how it should be
  ...
  SUBCOMPONENT(idex_reg, TYPE(MIPS_IDEX_UNOPT<XLEN>));
  ...
  bool finished() const override {
    ...
    for (int stage = IF; stage < STAGECOUNT; stage++) {
        ...
        /*
        /// IF - no fw no hz thus no NOP. Active path is always
        if(stage == 0 && stageInfo({0, stage}).stage_valid){
            pc_reg->out.setActivePath(true);
            pc_branch->out.setActivePath(true);
            pc_4->out.setActivePath(true);
            instr_mem->data_out.setActivePath(true);
        ...
        ///ID
        if(stage == 1 && stageInfo({0, stage}).stage_valid){
            decode->decode->instr_imm_15_0.setActivePath(true);
