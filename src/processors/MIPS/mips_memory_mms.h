...
class MIPS_Memory_MMS : public Component, public BaseMemory<true> {
public:
  SetGraphicsType(ClockedComponent);
  MIPS_Memory_MMS(const std::string &name, SimComponent *parent)
      : Component(name, parent) {
    addr >> mem->addr;
    wr_en >> mem->wr_en;
    data_in >> mem->data_in;
    addr_instr_mem >> instr_mem->addr;
  ...
  SUBCOMPONENT(mem, TYPE(MemoryAsyncRd<XLEN, c_MIPSInstrWidth>));
  SUBCOMPONENT(instr_mem, TYPE(ROM<XLEN, c_MIPSInstrWidth>));

  WIRE(wr_width, ceillog2(c_MIPSInstrWidth / 8 + 1)); // Write width, in bytes

  INPUTPORT(addr_instr_mem, XLEN);
  INPUTPORT(addr, XLEN);
  INPUTPORT(data_in, c_MIPSInstrWidth);
  INPUTPORT(wr_en, 1);
  INPUTPORT_ENUM(op, MIPS_MemOp); // original mem read
  INPUTPORT(mem_read, 1);
  OUTPUTPORT(data_out, c_MIPSInstrWidth);
};
