...
class MIPS_HazardUnit_Opt : public Component {
public:
  MIPS_HazardUnit_Opt(const std::string &name, SimComponent *parent)
      : Component(name, parent) {
    hazardIFIDEnable << [=] { return !hasHazard(); };
    hazardFEEnable << [=] { return !hasHazard(); };
    hazardIDEXEnable << [=] { return !hasEcallHazard(); };
    hazardEXMEMClear << [=] { return hasEcallHazard(); };
    hazardIDEXClear << [=] { return hasMEMIDLoadUseHazard(); };
  ...
  INPUTPORT_ENUM(wb_opcode, MIPS_Instr);
  ...
  OUTPUTPORT(hazardIFIDEnable, 1); //show only. Same functionality as hazardFEEnable
...
private:
  bool hasHazard() { return hasLoadUseHazard() || hasMEMIDLoadUseHazard() || hasEcallHazard(); }
  ...
  bool hasMEMIDLoadUseHazard() const {
    const unsigned exidx = ex_reg_wr_idx.uValue();
    const unsigned idx1 = id_reg1_idx.uValue();
    const unsigned idx2 = id_reg2_idx.uValue();
    ...
    if (id_opcode.uValue() == MIPS_Instr::BEQ ||
            id_opcode.uValue() == MIPS_Instr::BNE){
        if(opcode.uValue() == MIPS_Instr::NOP && mem_opcode.uValue() == MIPS_Instr::NOP)
            return 0;

        return (exidx == idx1 || exidx == idx2 || memidx == idx1 || memidx == idx2);
    }
    else if (id_opcode.uValue() == MIPS_Instr::BGEZ ||
            id_opcode.uValue() == MIPS_Instr::BGTZ ||
            id_opcode.uValue() == MIPS_Instr::BLEZ ||
            id_opcode.uValue() == MIPS_Instr::BLTZ){
        if(opcode.uValue() == MIPS_Instr::NOP && mem_opcode.uValue() == MIPS_Instr::NOP)
            return 0;

        return (exidx == idx1|| memidx == idx1);
    }
    else if (id_opcode.uValue() == MIPS_Instr::JALR ||
             id_opcode.uValue() == MIPS_Instr::JR){
        if(opcode.uValue() == MIPS_Instr::NOP && mem_opcode.uValue() == MIPS_Instr::NOP)
            return 0;
    ...
    else    return (exidx == idx1 || exidx == idx2) && mrd;
  }
  ...
  bool hasEcallHazard() const {
    ...
    const bool isEcall = opcode.uValue() == MIPS_Instr::SYSCALL;
