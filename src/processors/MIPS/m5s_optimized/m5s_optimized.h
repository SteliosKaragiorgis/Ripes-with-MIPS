...
#include "../../MIPS/mips_control_m5s_opt.h"
...
#include "../m5s/m5s_idex.h"
#include "../m5s/m5s_exmem.h"
#include "../m5s/m5s_memwb.h"

#include "m5s_hazardunit_opt.h"
#include "../m5s/m5s_forwardingunit.h"
...
class M5S_OPT : public RipesVSRTLProcessor {
...
public:
  ...
  M5S_OPT(const QStringList &extensions)
    ...
    hzunit->hazardIFIDEnable >> ifid_reg->enable;
    ...
    decode->instr_rs_25_21 >> idex_reg->reg1_idx_in;
    decode->instr_rt_20_16 >> idex_reg->reg2_idx_in;
    ...
    // -----------------------------------------------------------------------
    // ALU
    reg1_fw_src->out >> alu->op1;
    reg2_fw_src->out >> alu_op2_src->get(MIPS_AluSrc2::REG2);
    ...
    ifid_reg->pc4_out >> alu_branch->op1;
    ...
    memwb_reg->opcode_out >> hzunit->wb_opcode;
    ...
    idex_reg->reg1_idx_out >> funit->id_reg1_idx;
    idex_reg->reg2_idx_out >> funit->id_reg2_idx;
    exmem_reg->reg_do_write_out >> funit->mem_reg_wr_en;
    exmem_reg->write_reg_out >> funit->mem_reg_wr_idx;
    memwb_reg->reg_do_write_out >> funit->wb_reg_wr_en;
    memwb_reg->write_reg_out >> funit->wb_reg_wr_idx;
    idex_reg->opcode_out >> funit->opcode_ex;
    exmem_reg->opcode_out >> funit->opcode_mem;
    memwb_reg->opcode_out >> funit->opcode_wb;


    funit->alu_reg1_forwarding_ctrl >> reg1_fw_src->select;
    idex_reg->r1_out >> reg1_fw_src->get(MIPS_ForwardingSrc::IdStage);
    reg_wr_src->out >> reg1_fw_src->get(MIPS_ForwardingSrc::WbStage);
    exmem_reg->alures_out >> reg1_fw_src->get(MIPS_ForwardingSrc::MemStage);

    funit->alu_reg2_forwarding_ctrl >> reg2_fw_src->select;
    idex_reg->r2_out >> reg2_fw_src->get(MIPS_ForwardingSrc::IdStage);
    reg_wr_src->out >> reg2_fw_src->get(MIPS_ForwardingSrc::WbStage);
    exmem_reg->alures_out >> reg2_fw_src->get(MIPS_ForwardingSrc::MemStage);
  ...
  SUBCOMPONENT(control, MIPS_Control_M5S_OPT);
  ...
  SUBCOMPONENT(reg1_fw_src, TYPE(EnumMultiplexer<MIPS_ForwardingSrc, XLEN>));
  SUBCOMPONENT(reg2_fw_src, TYPE(EnumMultiplexer<MIPS_ForwardingSrc, XLEN>));
  ...
  SUBCOMPONENT(flush_or, TYPE(Or<1, 2>));
  ...
  // Hazard unit
  SUBCOMPONENT(hzunit, MIPS_HazardUnit_Opt);

  // Forwarding unit
  SUBCOMPONENT(funit, MIPS_ForwardingUnit);

  // Stage Registers
  SUBCOMPONENT(ifid_reg, TYPE(MIPS_IFID<XLEN>));
  SUBCOMPONENT(idex_reg, TYPE(M5S_IDEX<XLEN>));
  ...
  StageInfo stageInfo(StageIndex stage) const override {
    ...
    // clang-format off
        ...
        /// ID
            ...
            branch_eq->setCompActivePath(false);
            ...
            hzunit->hazardFEEnable.setActivePath(false);
            hzunit->hazardIFIDEnable.setActivePath(false);
            hzunit->hazardEXMEMClear.setActivePath(false);
            hzunit->hazardIDEXClear.setActivePath(false);
            ...
            if(stageValid){
                ...
                hzunit->hazardFEEnable.setActivePath(true);
                hzunit->hazardIFIDEnable.setActivePath(true);
                switch(hzunit->hazardEXMEMClear.uValue()){
                    case 1:{
                        hzunit->hazardEXMEMClear.setActivePath(true);
                        mem_stalled_or->out.setActivePath(true);
                        mem_stalled_or->setCompActivePath(true);

                        break;
                    }
                    default:{
                        hzunit->hazardEXMEMClear.setActivePath(false);
                        mem_stalled_or->out.setActivePath(false);
                        mem_stalled_or->setCompActivePath(false);

                        break;
                    }
                }

                switch(hzunit->hazardIDEXClear.uValue()){
                    case 1:{
                        hzunit->hazardIDEXClear.setActivePath(true);
                        break;
                    }
                    default:{
                        hzunit->hazardIDEXClear.setActivePath(false);
                ...
                switch(decode->opcode.uValue()){
                    ...
                    case MIPS_Instr::BEQ: case MIPS_Instr::BNE: {
                        ...
                        branch_eq->setCompActivePath(true);
        ...
        /// EX
            ...
            reg1_fw_src->setCompActivePath(false);
            reg2_fw_src->setCompActivePath(false);
            funit->setCompActivePath(false);

            idex_reg->pc4_reg->out.setActivePath(false);
            idex_reg->r1_reg->out.setActivePath(false);
            idex_reg->r2_reg->out.setActivePath(false);
            idex_reg->reg1_idx_reg->out.setActivePath(false);
            idex_reg->reg2_idx_reg->out.setActivePath(false);
            ...
            reg1_fw_src->out.setActivePath(false);
            reg2_fw_src->out.setActivePath(false);
            funit->alu_reg1_forwarding_ctrl.setActivePath(false);
            funit->alu_reg2_forwarding_ctrl.setActivePath(false);
            ...
            if(stageValid){
                ...
                idex_reg->reg1_idx_reg->out.setActivePath(true);
                idex_reg->reg2_idx_reg->out.setActivePath(true);
                ...
                //forward unit
                funit->alu_reg1_forwarding_ctrl.setActivePath(true);
                funit->alu_reg2_forwarding_ctrl.setActivePath(true);

                switch(funit->alu_reg1_forwarding_ctrl.uValue()){
                    case MIPS_ForwardingSrc::IdStage:{
                        idex_reg->r1_reg->out.setActivePath(true);
                        break;
                    }
                    case MIPS_ForwardingSrc::MemStage:{
                        exmem_reg->alures_reg->out.setActivePath(true);
                        break;
                    }
                    case MIPS_ForwardingSrc::WbStage:{
                ...
                switch(funit->alu_reg2_forwarding_ctrl.uValue()){
                    case MIPS_ForwardingSrc::IdStage:{
                ...
                switch(idex_reg->opcode_out.uValue()){
                    ...
                    case MIPS_Instr::XOR: case MIPS_Instr::MFHI: case MIPS_Instr::MFLO:{
                        ...
                        reg1_fw_src->setCompActivePath(true);
                        reg2_fw_src->setCompActivePath(true);
                        funit->setCompActivePath(true);

                        reg1_fw_src->out.setActivePath(true);
                        reg2_fw_src->out.setActivePath(true);
