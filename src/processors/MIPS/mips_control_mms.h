...
public:
  /* clang-format off */
    //RegDst
    static VSRTL_VT_U do_reg_dest_ctrl(const VSRTL_VT_U& state) {
        switch(state){
    ...
    static VSRTL_VT_U do_bne_write_ctrl(const VSRTL_VT_U& opc) {
        switch(opc){
            ...
            case MIPS_Instr::BLTZ: case MIPS_Instr::BNE:
                return 1;
            default:
                return 0;
        }
    ...
    //RegWrite
    static VSRTL_VT_U do_reg_do_write_ctrl(const VSRTL_VT_U& state) {
        switch(state) {
            case MIPSMulti_States::S4: case MIPSMulti_States::S7: case MIPSMulti_States::S11: case MIPSMulti_States::S12:
                return 1;
            default: return 0;
        }
    }
    //MemToReg
    static MIPS_RegWrSrc do_reg_wr_src_ctrl(const VSRTL_VT_U& state) {
        switch(state){
            // Load instructions
            case MIPSMulti_States::S0: case MIPSMulti_States::S7: case MIPSMulti_States::S11: case MIPSMulti_States::S12:
                return MIPS_RegWrSrc::ALURES;                  //0
    ...
    //ALUSrcA
    static MIPSMulti_AluSrc1 do_alu_op1_ctrl(const VSRTL_VT_U& state) {
        switch(state) {
            case MIPSMulti_States::S0: case MIPSMulti_States::S1:
                return MIPSMulti_AluSrc1::PC;                       //0
            default:        //S2, S6, S8
                return MIPSMulti_AluSrc1::REG1;                     //1
        }
    }
    //ALUSrcB
    static MIPSMulti_AluSrc2 do_alu_op2_ctrl(const VSRTL_VT_U& state) {
        switch(state) {

            case MIPSMulti_States::S6: case MIPSMulti_States::S8:
                return MIPSMulti_AluSrc2::REG2;     //00


            // Load/Store instructions
            case MIPSMulti_States::S0:
                return MIPSMulti_AluSrc2::FOUR;     //01

            case MIPSMulti_States::S1:
                return MIPSMulti_AluSrc2::IMMSHIFT; //11



            default:    //S2, S10
                return MIPSMulti_AluSrc2::IMM;      //10
        }
    }

    //ALUOp
    static MIPS_ALUOp do_alu_ctrl(const VSRTL_VT_U& state, const VSRTL_VT_U& opc) {
        switch(state){
            case MIPSMulti_States::S0: case MIPSMulti_States::S1: case MIPSMulti_States::S2:
                return MIPS_ALUOp::ADD;         //00
            ...
            switch(opc) {
                case MIPS_Instr::LUI:
                    return MIPS_ALUOp::LUI;
                case MIPS_Instr::ADDI: case MIPS_Instr::ADDIU:
                    return MIPS_ALUOp::ADD;
                case MIPS_Instr::SLTI:
                    return MIPS_ALUOp::LT;
                case MIPS_Instr::SLTIU:
                    return MIPS_ALUOp::LTU;
                case MIPS_Instr::XORI:
                    return MIPS_ALUOp::XOR;
                case MIPS_Instr::ORI:
                    return MIPS_ALUOp::OR;
                case MIPS_Instr::ANDI:
                    return MIPS_ALUOp::AND;
    ...
    //MemWrite
    static VSRTL_VT_U do_do_mem_write_ctrl(const VSRTL_VT_U& state) {
        switch(state) {
    ...
    //MemRead
    static VSRTL_VT_U do_do_read_ctrl(const VSRTL_VT_U& state) {
        switch(state) {
            case MIPSMulti_States::S0: case MIPSMulti_States::S3:
                return 1;
            default: return 0;
        }
    }
    //PCSource
    static MIPSMulti_PcSrc do_pc_source(const VSRTL_VT_U& state) {
        switch(state) {
            case MIPSMulti_States::S0:
                return MIPSMulti_PcSrc::ALU;        //00
            case MIPSMulti_States::S9: case MIPSMulti_States::S12:
                return MIPSMulti_PcSrc::JADD;       //10
            default:    //S8
            return MIPSMulti_PcSrc::ALUREG;    //01
        }
    }
    //IorD
    static MIPSMulti_Address do_iord(const VSRTL_VT_U& state) {

        switch(state) {
            case MIPSMulti_States::S0:
                return MIPSMulti_Address::PC;       //0
            default: //S3 S5
                return MIPSMulti_Address::ALUREG;      //1
        }
    }
    //IRWrite
    static VSRTL_VT_U do_irwrite(const VSRTL_VT_U& state) {
        switch(state) {
            case MIPSMulti_States::S0:
                return 1;
            default: return 0;
    ...
    //PCWrite
    static VSRTL_VT_U do_pc_write(const VSRTL_VT_U& state, const VSRTL_VT_U& opcode) {


        switch(state) {
            case MIPSMulti_States::S0: case MIPSMulti_States::S9: case MIPSMulti_States::S12: //case MIPSMulti_States::S8:
                return 1;
            case MIPSMulti_States::S7:
                switch(opcode){
    ...
    //PCWriteCond
    static VSRTL_VT_U do_pc_write_cond(const VSRTL_VT_U& state, const VSRTL_VT_U& opcode) {
        switch(state) {
            case MIPSMulti_States::S8:
                switch(opcode){
                    case MIPS_Instr::BEQ:
    ...
    static MIPSMulti_Instr do_instr_sel(const VSRTL_VT_U& state){
        switch(state){
            case MIPSMulti_States::S0:
                return MIPSMulti_Instr::S0;
            default:
                return MIPSMulti_Instr::OTHER;
        }
    }
...
public:
  MIPS_Control_MMS(const std::string &name, SimComponent *parent)
    ...
    reg_do_write_ctrl << [=] { return do_reg_do_write_ctrl(state.uValue()); };
    reg_wr_src_ctrl << [=] { return do_reg_wr_src_ctrl(state.uValue()); };
    alu_op1_ctrl << [=] { return do_alu_op1_ctrl(state.uValue()); };
  ...
  OUTPUTPORT_ENUM(alu_op1_ctrl, MIPSMulti_AluSrc1);
  OUTPUTPORT_ENUM(alu_op2_ctrl, MIPSMulti_AluSrc2);
  OUTPUTPORT_ENUM(pc_source, MIPSMulti_PcSrc);
  OUTPUTPORT_ENUM(iord, MIPSMulti_Address);
  OUTPUTPORT(irwrite, 1);
  OUTPUTPORT(pc_write, 1);
  OUTPUTPORT(pc_write_cond, 1);
  OUTPUTPORT_ENUM(alu_ctrl, MIPS_ALUOp);
  OUTPUTPORT_ENUM(instr_sel, MIPSMulti_Instr);
};
