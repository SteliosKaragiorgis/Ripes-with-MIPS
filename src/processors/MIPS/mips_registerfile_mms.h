...
class MIPS_RegisterFile_MMS : public Component {
public:
  SetGraphicsType(ClockedComponent);
  MIPS_RegisterFile_MMS(const std::string &name, SimComponent *parent)
    ...
    wr_en_0->out << [=] {
      ...
      switch(opcode.uValue()){
        ...
        case MIPS_Instr::JALR:
          switch(state.uValue()){
            case MIPSMulti_States::S7: case MIPSMulti_States::S12:
              return 1;
            default: return ret;
          }
        default:
