...
static std::optional<ClangFormatResult>
parseClangFormatOutput(const QString &cfOutput) {
  // The first line should contain a JSON formatted array detailing the remapped
  // cursor position and formatting status. If this is not the case, the
  // clang-format version might be outdated or something else went wrong.
...
void CodeEditor::onSave() {
  ...
  // We want to execute the formatter in the savefile's directory to adhere to
  // any formatting config files.
  ...
  const auto tempFileTemplate =
      QString(tmpDir.path() + QDir::separator() +
  ...
  if (tmpSrc.open()) {
    ...
    clangFormatArgs << "--cursor=" +
                           QString::number(this->textCursor().position());
    ...
    if (!stdErr.isEmpty()) {
      QMessageBox::warning(this, "Ripes",
                           "Error while executing formatter. Make sure the "
                           "formatter path points to a "
...
void CodeEditor::setupChangedTimer() {
  ...
  // A change in the document will start the timer - when the timer elapses, the
  // contents will be assembled if there is no syntax error. By doing this, the
  // timer is restartet each time a change occurs (ie. a user is continuously
  // typing).
  // An issue here is that QPlainTextEdit emits signals on text AND formatting
  // changes. To only get the text changes, we do as here
  // https://stackoverflow.com/a/22872958
...
int CodeEditor::lineNumberAreaWidth() {
  ...
  int space =
      rightPadding + fontMetrics().horizontalAdvance(QString("1")) * digits;
  return space;
}
...
static const QStringList c_indentStartCharacters =
    QStringList{":", "(", "{", "["};

void CodeEditor::keyPressEvent(QKeyEvent *e) {
  const unsigned indentAmt =
      RipesSettings::value(RIPES_SETTING_INDENTAMT).toInt();

  const auto preCursorText =
      textCursor().block().text().left(textCursor().positionInBlock());
  const auto postCursorText = textCursor().block().text().right(
      textCursor().block().length() - textCursor().positionInBlock() - 1);
  const QString preCursorChar =
      preCursorText.isEmpty() ? QString() : *preCursorText.rbegin();
  const QString postCursorChar =
      postCursorText.isEmpty() ? QString() : *postCursorText.begin();

  /**
   * The following is a collection of quality-of-life changes to the behaviour
   * of the editor, mimmicking features generally found in IDEs.
   */
  if (c_closingBrackets.count(e->text()) && postCursorChar == e->text()) {
    // Skip closing bracket insertion if already present after the current
    // character
    auto cursor = textCursor();
    cursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor);
    setTextCursor(cursor);
  } else if (c_bracketPair.count(e->text()) &&
             (postCursorChar == " " || postCursorChar.isEmpty())) {
    ...
    // If this is the start of a bracket, the remainder of the line text is to
    // be moved to the 2nd line following the current cursor position. At this
    // line we maintain the current indentation level.
    auto bracketCharIt = c_bracketPair.find(preCursorChar);
    if (bracketCharIt != c_bracketPair.end() &&
        bracketCharIt->second == postCursorChar) {
      postIndent = indent;
    }

    // Add additional indent if last character was an indent start character,
    // e.g., a bracket
    ...
    } else if (!preCursorChar.isEmpty() && !postCursorChar.isEmpty() &&
               c_bracketPair.count(preCursorChar) &&
...
void CodeEditor::setSourceType(SourceType type,
  ...
  case SourceType::Assembly: {
    ...
    if (isa->isaID() == ISA::RV32I || isa->isaID() == ISA::RV64I || isa->isaID() == ISA::MIPS32I) {
